...
export default function Artikel(): JSX.Element {
  ...
  const filteredPosts = activeTab === "Semua" ? PostsList : PostsList.filter((post) => post.category === activeTab);
    ...
    <section className="mx-auto mb-12 mt-6 w-[90%]">
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
        {filteredPosts.slice(0, visiblePosts).map((post) => (
          <article key={post.id} className="group overflow-hidden rounded-xl bg-white shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
      ...
      {visiblePosts < filteredPosts.length && (
        <div className="mt-12 flex justify-center">
          <button onClick={handleLoadMore} className="rounded-lg bg-emerald-600 px-6 py-3 font-medium text-white transition-all duration-300 hover:bg-emerald-700">
            Muat Lebih Banyak
          </button>
        </div>
      )}
