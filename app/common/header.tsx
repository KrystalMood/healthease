...
export default function Header(): JSX.Element {
  ...
  const handleScroll = useCallback(createScrollHandler(setIsScrolled, setIsNotTop), []);
    ...
    >
      <div className="mx-auto flex w-[90%] items-center justify-between">
        <Link to="/" className="text-2xl font-bold tracking-tight text-emerald-600">
          <img src={isScrolled ? "/icon.png" : "/iconScroll.png"} alt="Logo Healthease" className="w-48" />
        </Link>
        <nav aria-label="Main navigation" className="hidden items-center justify-between gap-x-14 lg:flex">
          {MENU_ITEMS.map((menu) => (
            <Link
              key={menu}
              to={getLinkPath(menu)}
              className={getMenuItemClassName(isScrolled, location.pathname === getLinkPath(menu))}
            >
              {menu}
            </Link>
          ))}
        ...
        <section className="hidden items-center justify-between gap-x-4 lg:flex">
          <Link to="/daftar" className={`border-1 rounded-md border border-emerald-500 px-6 py-2 text-sm font-semibold shadow-md transition-all duration-300 hover:scale-105 hover:bg-slate-50/25 ${isScrolled ? "text-slate-950" : "text-white"}`}>
            Daftar
          </Link>
