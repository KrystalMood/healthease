...
export default function Hero(): JSX.Element {
  return (
    <section className="mx-auto mt-40 flex w-[90%] gap-x-12">
      <div className="flex flex-col">
        <div className="mx-auto flex w-fit items-center rounded-xl bg-emerald-50 px-4 py-4 shadow-sm lg:mx-0">
        ...
        <div className="mx-auto mt-8 flex gap-4 lg:mx-0">
          <Link
            to={`/reservasi`}
            className="rounded-lg bg-emerald-600 px-6 py-4 font-medium text-white transition-all duration-500 ease-in-out hover:scale-[1.05] hover:bg-emerald-800"
          >
            Buat Janji Temu
          </Link>
          <Link
            to={`/layanan`}
            className="rounded-lg border-2 border-emerald-600 bg-white px-6 py-4 font-medium text-emerald-700 transition-all duration-200 hover:scale-105 hover:bg-emerald-50"
          >
            Lihat Layanan Kami
          </Link>
        </div>
      ...
      <img
        ...
        className="hidden h-96 rounded-xl [box-shadow:_1rem_1rem_0.75rem_rgb(0_0_0_/_0.25)] lg:block"
      />
    </section>
  );
}